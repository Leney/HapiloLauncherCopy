...
public class PersonRankingAdapter extends BaseRecyclerViewAdapter<List<ActivityDereeListBean.RecordsBean>,PersonRankingAdapter.ViewHolder> {
    private List<ActivityDereeListBean.RecordsBean> list;
    PersonRankingAdapter(){
        this.list=new ArrayList<>();
    }
    @Override
    public void addData(List<ActivityDereeListBean.RecordsBean> personRanking_entities) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ActivityDereeListBean.RecordsBean entity=list.get(position);
        switch (position){
            case 0:
