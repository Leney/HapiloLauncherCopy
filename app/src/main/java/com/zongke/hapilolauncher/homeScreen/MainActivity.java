...
import com.zongke.hapilolauncher.db.entity.ActivityDereeListBean;
...
public class MainActivity extends RxAppCompatActivity implements View.OnClickListener, LauncherRotatingMenu.LauncherRotatingMenuCallBack, LoaderManager.LoaderCallbacks<Cursor>, ClientListener {
    ...
    private void executeNetWork() {
        ...
        Subscription subscription = this.retrofitProvider.getActivityDreeList(BuilderMap.builderActivityList(userInfo.getToken(), 1, 3),
                new ResponseResultListener<ActivityDereeListBean>() {
                    @Override
                    public void success(ActivityDereeListBean activityDereeListBean) {
                        adapter.addData(activityDereeListBean.getRecords());
                    }
    ...
    private void queryInstallPackage() {
        Subscription subscription = Observable.create(subscriber -> {
            Cursor cursor = SQLBriteProvider.getInstance(this.getApplicationContext())
                    .getBriteDatabase()
                    .query("select * from " + ContentProviderConfig.TABLE_NAME_COLLECTION_APP, new String[]{});
            subscriber.onNext(cursor);
        }).compose(SubscribeUtils.createTransformer()).subscribe(result ->
    ...
    private void addChildView() {
        mainMessageFragment = MainMessageFragment.Companion.newInstance();
        getSupportFragmentManager().beginTransaction().add(R.id.main_change_layout, mainMessageFragment, MainMessageFragment.Companion.getTAG()).commitAllowingStateLoss();
    }
    ...
    private void initView() {
        ...
        findViewById(R.id.launcher_user_icon).setOnClickListener(v -> {
    ...
    private AlertDialog createDialog() {
        ...
        builder.setNegativeButton("取消", (dialog, which) -> cancelAgainLoginDialog());
    ...
    private void addData(final List<LauncherRotatingMenu.TestEntity> list) {
        subscription = Observable.create(subscriber -> {
            ...
            for (LauncherRotatingMenu.TestEntity testEntity : list) {
                Bitmap icon_bitmap = AppFilterUtil.filterApp(BaseApplication.Companion.getInstance(), testEntity.packageName);
            ...
            subscriber.onNext(testEntityList);
                ...
                .subscribe(functionList_entity -> {
                            launcherRotatingMenu.setLocalBitmapSize(16 - list.size());
                            launcherRotatingMenu.addData((List<LauncherRotatingMenu.TestEntity>) functionList_entity);
                        }
                );
        this.compositeSubscription.add(subscription);
    }
        ...
        switch (requestCode) {
            case PermissionUtils.SET_PREFERRED_APPLICATIONS_CODE:
                if (grantResults.length > 0 && grantResults[0]
                    ...
                    LauncherUtils.Companion.getInstance().setDefaultLauncher(BaseApplication.Companion.getInstance(), this.getClass().getSimpleName());
                } else {//未授权
                    ToastUtils.Companion.showToast(getApplicationContext(), "请开启设置Launcher权限");
                }
                break;
            default:
                break;
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        SystemUIManager.INSTANCE.setStickyStyle(getWindow());
