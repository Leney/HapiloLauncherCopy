...
/*    public static Observable<HttpResult<DeviceMSG>> createIntervalQueryDeviceSafe() {
     ...
     * @return
     */
    public static Observable<Bitmap> createScanCode(String content, int width, int height, int bitmapId) {
        return Observable.create(subscriber -> {
            Bitmap logoBitmap = BitmapUtils.decodeBitmapResource(BaseApplication.Companion.getInstance(), bitmapId);
