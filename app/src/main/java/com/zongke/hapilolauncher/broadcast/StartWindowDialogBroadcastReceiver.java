    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (messageType){
            case TYPE_SHOW_CONTENT:
                if (! SuspensionWindowManagerUtils.Companion.windowIsOpen()){
                    SuspensionWindowManagerUtils.Companion.createSuspensionWindow(context.getApplicationContext(), R.layout.windowdialog_test_layout);
                    if(!TextUtils.isEmpty(intent.getStringExtra("content"))){
                        SuspensionWindowManagerUtils.Companion.getSuspensionWindowWidget().showSpecifiedContent(intent.getStringExtra("content"));
                    }
                }
                break;
            default:
